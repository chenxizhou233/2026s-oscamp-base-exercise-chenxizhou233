...
pub async fn producer_consumer(items: Vec<String>) -> Vec<String> {
    ...
    producer.await.expect("produce panic");
...
pub async fn fan_in(n_producers: usize) -> Vec<String> {
    ...
    let (tx, mut rx) = mpsc::channel(n_producers + 1);
    let mut producers = vec![];
    for id in 0..n_producers {
        let tx = tx.clone(); // In Rust, remember to clone before using something that will be removed.
        producers.push(tokio::spawn(async move {
            if tx.send(format!("producer {id}: message")).await.is_err() {}
        }));
    }
    drop(tx);
    let consumer = tokio::spawn(async move {
