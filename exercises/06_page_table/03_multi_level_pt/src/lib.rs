...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let vpn2 = Sv39PageTable::extract_vpn(va, 2);
        let vpn1 = Sv39PageTable::extract_vpn(va, 1);
        let vpn0 = Sv39PageTable::extract_vpn(va, 0);
        let mut curr = self.root_ppn;
        let pte2 = self.nodes[&curr].entries[vpn2];
        if (pte2 & PTE_V) == 0 {
            let child = self.alloc_node();
            self.nodes.get_mut(&curr).unwrap().entries[vpn2] = child << PPN_SHIFT | PTE_V;
            curr = child;
        } else {
            curr = (pte2 >> PPN_SHIFT);
        }
        let pte1 = self.nodes.get(&curr).unwrap().entries[vpn1];
        if (pte1 & PTE_V) == 0 {
            let child = self.alloc_node();
            self.nodes.get_mut(&curr).unwrap().entries[vpn1] = child << PPN_SHIFT | PTE_V;
            curr = child;
        } else {
            curr = (pte1 >> PPN_SHIFT);
        }
        self.nodes.get_mut(&curr).unwrap().entries[vpn0] = ((pa >> 12) << PPN_SHIFT) | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        let vpn0 = Sv39PageTable::extract_vpn(va, 0);
        let mut curr = self.root_ppn;
        let pte2 = self.nodes.get(&curr).unwrap().entries[vpn2];
        if (pte2 & PTE_V) == 0 {
            return TranslateResult::PageFault;
        }
        if (pte2 & (PTE_R | PTE_W | PTE_X)) != 0 {
            return TranslateResult::Ok(((pte2 >> PPN_SHIFT) << 12) | (va & 0x3FFF_FFFF));
        }
        curr = pte2 >> PPN_SHIFT;
        ...
        if (pte1 & (PTE_R | PTE_W | PTE_X)) != 0 {
            return TranslateResult::Ok(((pte1 >> PPN_SHIFT) << 12) | (va & 0x1F_FFFF));
        }
        curr = pte1 >> PPN_SHIFT;

        let pte0 = self.nodes.get(&curr).unwrap().entries[vpn0];
        if (pte0 & PTE_V) == 0 {
            return TranslateResult::PageFault;
        }
        TranslateResult::Ok(((pte0 >> PPN_SHIFT) << 12) | (va & 0xFFF))
    }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        self.nodes.get_mut(&curr).unwrap().entries[vpn1] = ((pa >> 12) << PPN_SHIFT | flags);
    }
}
