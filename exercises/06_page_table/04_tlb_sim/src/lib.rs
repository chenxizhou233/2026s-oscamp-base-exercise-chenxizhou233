...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        ...
        for item in entries {
            if item.valid && item.vpn == vpn && item.asid == asid {
                self.stats.hits += 1;
                return Some(item.ppn);
            }
        }
        self.stats.misses += 1;
        None
    ...
    pub fn flush_by_asid(&mut self, asid: u16) {
        ...
        for entry in &mut self.entries {
            entry.valid = if (entry.asid == asid) {
                false
            } else {
                entry.valid
            };
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        // TODO: 实现 TLB + 页表的二级查找
        match self.tlb.lookup(vpn, self.current_asid) {
            Some(v) => return Some(v),
            None => {
                for (id, page) in &self.page_table {
                    if (self.current_asid == *id && vpn == page.vpn) {
                        self.tlb.insert(page.vpn, page.ppn, *id, page.flags);
